...
class TestMovieClassifier(unittest.TestCase):
    ...
    def test_classify_by_similar_movies(self):
        ...
        similar_movies = self.movie_classifier.classify_by_similar_movies(genre='Drama', actor=None)
        ...
        similar_movies = self.movie_classifier.classify_by_similar_movies(genre=None, actor='Al Pacino')
    ...
    def test_classify_by_same_actor(self):
        movie1 = Movie(title='The Matrix', imdb_rating=8.7, genres=['Action', 'Sci-Fi'], actors=['Keanu Reeves', 'Laurence Fishburne'])
        movie2 = Movie(title='The Matrix Reloaded', imdb_rating=7.2, genres=['Action', 'Sci-Fi'], actors=['Keanu Reeves', 'Laurence Fishburne'])
        movie3 = Movie(title='John Wick', imdb_rating=7.4, genres=['Action', 'Thriller'], actors=['Keanu Reeves'])
        movie4 = Movie(title='The Terminator', imdb_rating=8.0, genres=['Action', 'Sci-Fi'], actors=['Arnold Schwarzenegger'])
        movie5 = Movie(title='Terminator 2: Judgment Day', imdb_rating=8.5, genres=['Action', 'Sci-Fi'], actors=['Arnold Schwarzenegger'])
        ...
        same_actor_movies = classifier.classify_by_same_actor(actor='Arnold Schwarzenegger')
